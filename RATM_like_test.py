...
test = T.function([seq_len_scalar, imgs, starts, startAtt, targets], [cost, bbox_seq, att_seq], allow_input_downcast=True)
import cPickle
...
bmnist = BouncingMNIST(nr_objs, _len, batch_size, img_row, dataset_name+"/inputs", dataset_name+"/targets", acc=acc_scale, scale_range=zoom_scale, clutter_move = clutter_move, with_clutters = with_clutters, buff=False)
try:
	_iou=[]
	for i in range(0, 1):
		for j in range(0, 1000):
			...
			print 'Attention, sigma, stride\n', NP.mean(NP.abs(att_seq), axis=1)
			...
			print i, j, cost
			iou = intersect / union
			print 'mean iou on batch axis = ', NP.average(iou, axis=1)
			_iou.append(NP.mean(iou,axis=1))
