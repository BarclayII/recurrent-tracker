...
W_fc3 = T.shared(glorot_uniform((gru_dim, 1)), name='W_fc2')
b_fc3 = T.shared(NP.zeros((1,), dtype=T.config.floatX), name='b_fc2')

### NETWORK PARAMETERS END

print 'Building network'

def gauss2D(shape, u, sigma):
    """
...
def _step(img, prev_bbox, prev_att, state):
	...
	for i in xrange(batch_size):
		g2D=gauss2D((img_row, img_col), (cy[i], cx[i]), (sigma[i], sigma[i]))
...
try:
	for i in range(0, 50):
		for j in range(0, 2000):
			...
			att = np.zeros(np.shape(label[:,0,0:1]))
			att[:,0] = 1000
			cost, bbox_seq, att_seq = train(_len, data, label[:, 0, :], att, label)
			print 'Attention, sigma', NP.mean(NP.abs(att_seq), axis=1)
